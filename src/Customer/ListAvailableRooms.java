...
import Beans.Room;
	...
	public class ListAvailableRooms implements ActionListener {
		...
		int meal;
		ResultSet Rs[];
		ArrayList<Room> SelectedRooms; //for each of the selected room generate a booking with same ID
		ArrayList<String> RoomsAvailable; 
		JButton bookSelected;
		BookingDetail B;
		
		
		public ListAvailableRooms(boolean t1,boolean t2,boolean t3,boolean t4,boolean t5,BookingDetail B)
		{
			Rs = new ResultSet[5];
			roomType = new boolean[5];
			roomType[0] = t1;
			roomType[1] = t2;
			roomType[2] = t3;
			roomType[3] = t4;
			...
			bookSelected = new JButton("Book Selected Rooms");
			RoomsAvailable = new ArrayList<String>();
			createFrame();
		}
		public void createFrame()
		{
			...
			JLabel note = new JLabel("Only White Rooms are available. Others are not. \n The Rooms you select turn ORANGE");
			note.setBounds(200,35,400,100);
			...
			for(int x = 71,i=0; i<10; x+=116)
			{
				...
				F1.singleBedroomButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<10; x+=116)
			{
				...
				F1.doubleBedroomButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<10; x+=116)
			{
				...
				F2.singleBedroomButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<10; x+=116)
			{
				...
				F2.doubleBedroomButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<10; x+=116)
			{
				...
				F3.suiteButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<5; x+=116)
			{
				...
				F4.deluxSuiteButtons[i].addActionListener(this);
			...
			for(int x = 71,i=0; i<1; x+=116)
			{
				...
				F5.penthouseSuiteButtons[i].addActionListener(this);
				bottomPanel.add(F5.penthouseSuiteButtons[i]);
				i++;
			}
		
			getRooms(); //gets available rooms and puts them RoomAvailable<String>
			fillColor(); //fills white color in available rooms
			

			bookSelected.setBounds(1000, 471, 200, 35);
			bookSelected.addActionListener(this);
			backButton.addActionListener(this);
			bottomPanel.add(bookSelected);
		...
		public void getRooms()
		{
			...
			try {
				...
				while(Rs[0].next())
				{
					RoomsAvailable.add("Room "+Rs[0].getString(1));
				}
				while(Rs[1].next())
				{
					RoomsAvailable.add("Room "+Rs[1].getString(1));
				}
				while(Rs[2].next())
				{
					RoomsAvailable.add("Room "+Rs[2].getString(1));
				}
				while(Rs[3].next())
				{
					RoomsAvailable.add("Room "+Rs[3].getString(1));
				}
				while(Rs[4].next())
				{
					RoomsAvailable.add("Room "+Rs[4].getString(1));
				}
		...
		public void fillColor()
		{
			...
			for(int i=0; i<10; i++)
			{
				...
				if(RoomsAvailable.contains(F1.singleBedroomButtons[i].getText()))
					F1.singleBedroomButtons[i].setBackground(Color.white);

				if(RoomsAvailable.contains(F1.doubleBedroomButtons[i].getText()))
					F1.doubleBedroomButtons[i].setBackground(Color.white);
				
				if(RoomsAvailable.contains(F2.singleBedroomButtons[i].getText()))
					F2.singleBedroomButtons[i].setBackground(Color.white);
				
				if(RoomsAvailable.contains(F2.doubleBedroomButtons[i].getText()))
					F2.doubleBedroomButtons[i].setBackground(Color.white);
			
				if(RoomsAvailable.contains(F3.suiteButtons[i].getText()))
					F3.suiteButtons[i].setBackground(Color.white);
			...
			for(int i=0; i<5; i++)
			{
				...
				if(RoomsAvailable.contains(F4.deluxSuiteButtons[i].getText()))
					F4.deluxSuiteButtons[i].setBackground(Color.white);
				
			}
				
				
				if(RoomsAvailable.contains("Room 501"))
					F5.penthouseSuiteButtons[0].setBackground(Color.white);
		...
		public void actionPerformed(ActionEvent Ae) {
			...
			if(temp.getBackground() == Color.WHITE && RoomsAvailable.contains(temp.getText()))
			{
				int roomno = Integer.parseInt(temp.getText().substring(5));
				try {
					R.setAvailability(roomno, 0);
					temp.setBackground(Color.orange);
					Room selected = R.getRoom(roomno);
			...
			if(Ae.getSource() == bookSelected)
			{
				if(SelectedRooms.isEmpty())
					JOptionPane.showMessageDialog(this.frame, "NO ROOMS SELECTED!!");
				else
				{
					try {
						new Bookavailable(this.B,roomType).frame.setVisible(true);
